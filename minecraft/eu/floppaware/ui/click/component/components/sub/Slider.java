...
public class Slider extends Component {
	...
	public void renderComponent() {
		Gui.drawRect(parent.parent.getX() + 2, parent.parent.getY() + offset, parent.parent.getX() + parent.parent.getWidth(), parent.parent.getY() + offset + 12, this.hovered ? 0xFF222222 : 0xFF111111);
		 final int drag = (int)(this.set.getCurrentValue() / this.set.getMaximum() * this.parent.parent.getWidth());
		...
		Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(this.set.getName() + ": " + this.set.getCurrentValue() , (parent.parent.getX()* 2 + 15), (parent.parent.getY() + offset + 2) * 2 + 5, -1);
	...
	public void updateComponent(int mouseX, int mouseY) {
		...
		renderWidth = (88) * (set.getCurrentValue() - min) / (max - min);
		
		if (dragging) {
			if (diff == 0) {
				set.setCurrentValue(set.getMinimum());
			}
			else {
				double newValue = roundToPlace(((diff / 88) * (max - min) + min), 2);
				set.setCurrentValue((float) newValue);
			}
		}
	}
